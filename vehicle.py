...
import os
from tkinter import HORIZONTAL, Scale, PhotoImage
...
class Vehicle:
    def __init__(self):
        ...
        archi1=PhotoImage(file="coche1.png")
        self.img1 = self.canvas1.create_image(0,0, image=archi1, anchor="nw")
        
        archi2=tk.PhotoImage(file="coche2.png")
        self.img2 = self.canvas1.create_image(0,0, image=archi2, anchor="nw", state='hidden')

        self.estadoMotor = ttk.LabelFrame(self.ventana1, text="Estado del motor", width=665, height=55)
        self.estadoMotor.grid(column=1, row=2)
        self.motorDatos = tk.Label(self.estadoMotor, text=self.engine, padx=15, pady=15)
        self.motorDatos.pack()

        self.estadoLuminosidad = tk.LabelFrame(self.ventana1, text="Luminosidad", width=665, height=55)
        self.estadoLuminosidad.grid(column=1, row=4)
        self.lumScale = tk.Scale(self.estadoLuminosidad, from_=0, to=100, orient=tk.HORIZONTAL, length=665, command=self.actualizaLum)
        self.lumScale.set(50)
        self.lumScale.grid(column=0, row=1, sticky='we')
        self.lumScale.pack()

        self.labelFuel = ttk.LabelFrame(self.ventana1, text="Nivel de combustible")
        self.labelFuel.grid(column=1, row=5)
        self.barraFuel = ttk.Progressbar(self.labelFuel,orient=HORIZONTAL, length=665, mode='determinate')
        self.barraFuel.pack()
    ...
    def actualizaFuel(self):
        self.barraFuel['value'] = self.fuel.get_porcentage_level()

    def actualizaLum(self,valor):
        self.environment.set_lum(int(valor))
    ...
    def action(self,evento): 
        if evento.keysym=='s':
                self.blinker_front.marcador = not self.blinker_front.marcador
        if evento.keysym=='a':
                self.blinker_rear.marcador = not self.blinker_rear.marcador
        if evento.keysym=='r':
    ...
    def do_work(self):
        ...
        self.light.update()
        self.fuel.update()
        self.motorDatos.config(text=self.engine)
